...
Эта команда требует папку `health_diet_data` в исходном коде. Путь можно передать через `HEALTH_DIET_DATA_DIRECTORY`; это нужно для одноразового запуска в Docker. Папка не входит в образ и не читается при обычном запуске бота.
