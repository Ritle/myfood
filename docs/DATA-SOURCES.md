...
В системный каталог также загружается 12 975 позиций из JSON-файлов, помещённых владельцем проекта в папку [`health_diet_data`](../health_diet_data). Для них используется `source = HEALTH_DIET`. Поле `source_ref` создаётся из нормализованного названия и КБЖУ, поэтому повторный импорт обновляет запись, а одинаковые строки из разных файлов не дублируются.
...
Все источники загружаются командой:
